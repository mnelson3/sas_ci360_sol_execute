# sas_ci360_sol_execute

Historical Python client for SAS Customer Intelligence 360 **Marketing Execution** APIs, retained as a reference. It is part of the `sas-ci360` family of solution packages, alongside packages for planning, identity, data, content delivery, and workflow.
...
`sasci360solexecute` wraps the CI 360 Marketing Execution REST API with a thin `Execute` class built on top of the shared `sasci360apicore` (connection, encryption, logging) and `sasci360apimarketingexecution` packages. It's meant to be dropped into automation scripts or scheduled jobs that need to drive CI 360 marketing execution jobs programmatically rather than through the UI.
...
Key dependencies: `pandas`, `PyJWT`, `requests`, `saspy`, `schedule`, `DateTime`, `urllib3`, plus the sibling `sasci360apicore` and `sasci360apimarketingexecution` packages.
...
```python
from sasci360solexecute import Execute

execute = Execute(
    algorithm="HS256",
    api="marketing-execution",
    encoding="utf-8",
